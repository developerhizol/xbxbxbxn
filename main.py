...
from config import BOT_TOKEN, CONFIG_URL, PREMIUM_CONFIG_URL, ADMIN_ID, SUBGRAM_API_KEY, SUBGRAM_CHECK_URL, PLATEGA_MERCHANT_ID, PLATEGA_SECRET, PLATEGA_API_URL, REFERRAL_BOT_URL
from utils import (
...
async def get_device_instruction_text(device, language: str, plan: str = 'free', user_id: int = None):
    ...
    # Проверяем реальный статус премиума
    is_premium = False
    if user_id:
        is_premium = check_premium_active(user_id)
    
    # Выбираем конфиг в зависимости от реального статуса премиума
    if is_premium:
        config_url = PREMIUM_CONFIG_URL
    else:
...
async def set_language_callback(callback: types.CallbackQuery):
    ...
    # Исправлено: используем правильный путь к БД
