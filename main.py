...
from config import BOT_TOKEN, CONFIG_URL, ADMIN_ID, SUBGRAM_API_KEY, SUBGRAM_CHECK_URL, PLATEGA_MERCHANT_ID, PLATEGA_SECRET, PLATEGA_API_URL, REFERRAL_BOT_URL, SUBSCRIBE_URL
from utils import (
    ...
    force_sync_all_users, auto_disable_expired_premium, save_temp_transaction, get_temp_transaction,
...
# БАЗА ЗАДАНИЙ для всех эмодзи (с правильными склонениями)
TASKS_DB = {
...
async def get_payment_keyboard(language: str, transaction_id: str, paid: bool = False, is_replenish: bool = False):
    builder = InlineKeyboardBuilder()
    if language == "ru":
        if paid:
            if is_replenish:
                builder.row(InlineKeyboardButton(
                    text=f"Подтвердить",
                    callback_data=f"confirm_replenish_{transaction_id}",
                    style="success"
                ))
            else:
                builder.row(InlineKeyboardButton(
                    text=f"Активировать премиум",
                    callback_data=f"activate_premium_{transaction_id}",
                    style="success",
                    icon_custom_emoji_id=CROWN_EMOJI_ID
                ))
        else:
            builder.row(InlineKeyboardButton(
                text=f"Оплатить",
                url=f"https://app.platega.io/pay/{transaction_id}",
                style="success",
                icon_custom_emoji_id=CARD_EMOJI_ID
            ))
            builder.row(InlineKeyboardButton(
                text=f"Отмена",
...
async def get_premium_payment_keyboard(language: str, transaction_id: str, paid: bool = False):
    ...
    if language == "ru":
        if paid:
            builder.row(InlineKeyboardButton(
                text=f"Активировать премиум",
                callback_data=f"activate_premium_internal_{transaction_id}",
                style="success",
                icon_custom_emoji_id=CROWN_EMOJI_ID
            ))
        else:
            ...
            builder.row(InlineKeyboardButton(
                text=f"Отмена",
                callback_data="cancel_premium_payment",
                style="danger",
                icon_custom_emoji_id=CROSS_PAY_EMOJI_ID
            ))
    else:
...
async def get_replenish_payment_text(language: str, amount: int, transaction_id: str, paid: bool = False):
    if language == "ru":
        if paid:
            text = (f"{MONEY_FLY} <b>Ваш счет для пополнения баланса успешно оплачен</b>\n\n"
            ...
            text = (f"{MONEY_FLY} <b>Ваш счет для пополнения баланса успешно создан</b>\n\n"
                    f"{GLOBE} <b>Провайдер:</b> Platega\n\n"
                    f"{MONEY_BAG} <b>Сумма пополнения:</b> <code>{amount} RUB</code>\n"
                    f"{ID_CARD_PAY} <b>ID транзакции:</b> <code>{transaction_id}</code>\n\n"
        ...
        if paid:
            text = (f"{MONEY_FLY} <b>Your balance recharge invoice has been paid</b>\n\n"
            ...
            text = (f"{MONEY_FLY} <b>Your balance recharge invoice has been created</b>\n\n"
                    f"{GLOBE} <b>Provider:</b> Platega\n\n"
                    f"{MONEY_BAG} <b>Recharge amount:</b> <code>{amount} RUB</code>\n"
                    f"{ID_CARD_PAY} <b>Transaction ID:</b> <code>{transaction_id}</code>\n\n"
...
async def get_device_instruction_text(device, language: str, plan: str = 'free', user_id: int = None):
    device_emojis = {
        "iphone": IPHONE,
        "android": ANDROID,
        "macos": MACOS,
    ...
    if plan == 'premium' and user_id and check_premium_active(user_id):
        # Ссылка для импорта в Happ
        config_url = f"happ://add?url={SUBSCRIBE_URL}/{user_id}"
    else:
        config_url = CONFIG_URL
    
    if language == "ru":
        text = (f"<b>Инструкция для</b> {device_emoji} <b>{device_name}:</b>\n\n"
        ...
        text = (f"<b>Instructions for</b> {device_emoji} <b>{device_name}:</b>\n\n"
...
async def get_device_instruction_keyboard(device, language: str, plan: str = 'free', user_id: int = None):
    builder = InlineKeyboardBuilder()
    ...
    if language == "ru":
        builder.row(
            InlineKeyboardButton(
                text=f"Скачать",
                url=DOWNLOAD_LINKS.get(device, DOWNLOAD_LINKS["iphone"]),
                style="primary",
                icon_custom_emoji_id=NUMBER_1_ID
            ),
            InlineKeyboardButton(
                text=f"Подключиться",
                url=config_url,
                style="success",
                icon_custom_emoji_id=NUMBER_2_ID
            )
        )
        builder.row(
            InlineKeyboardButton(
    ...
    if language == "ru":
        builder.row(
            InlineKeyboardButton(
                ...
                style="success",
                icon_custom_emoji_id=CHECK_MARK_EMOJI_ID
            ),
            InlineKeyboardButton(
                text="Нет, не хочу",
                callback_data="cancel_broadcast",
...
async def get_captcha_keyboard(emojis_list: list):
    """Создает клавиатуру для капчи (5x2, по 5 кнопок в строке)"""
    builder = InlineKeyboardBuilder()
    builder.row(
        InlineKeyboardButton(text=emojis_list[0], callback_data=f"captcha_{emojis_list[0]}"),
        InlineKeyboardButton(text=emojis_list[1], callback_data=f"captcha_{emojis_list[1]}"),
        InlineKeyboardButton(text=emojis_list[2], callback_data=f"captcha_{emojis_list[2]}"),
        InlineKeyboardButton(text=emojis_list[3], callback_data=f"captcha_{emojis_list[3]}"),
        InlineKeyboardButton(text=emojis_list[4], callback_data=f"captcha_{emojis_list[4]}")
    )
    builder.row(
        InlineKeyboardButton(text=emojis_list[5], callback_data=f"captcha_{emojis_list[5]}"),
        InlineKeyboardButton(text=emojis_list[6], callback_data=f"captcha_{emojis_list[6]}"),
        InlineKeyboardButton(text=emojis_list[7], callback_data=f"captcha_{emojis_list[7]}"),
        InlineKeyboardButton(text=emojis_list[8], callback_data=f"captcha_{emojis_list[8]}"),
        InlineKeyboardButton(text=emojis_list[9], callback_data=f"captcha_{emojis_list[9]}")
    )
    return builder.as_markup()

async def create_platega_payment(amount: int, user_id: int, description: str = "Premium access"):
    headers = {
    ...
    payload = {
        ...
        "return": f"{REFERRAL_BOT_URL}",
        "failedUrl": f"{REFERRAL_BOT_URL}",
...
async def start_captcha(message: types.Message, user_id: int, referrer_id: int, language: str):
    # Выбираем 10 случайных эмодзи из всей базы
    selected_emojis = random.sample(ALL_EMOJIS, 10)
    # Выбираем случайный эмодзи из выбранных как задание
    target_emoji = random.choice(selected_emojis)
    
    # Получаем текст задания в зависимости от языка
    if language == "ru":
...
async def handle_captcha(callback: types.CallbackQuery):
    ...
    language = get_language(user_id) or "ru"
...
async def cmd_stats(message: types.Message):
    ...
    language = get_language(ADMIN_ID) or "ru"
    if language == "ru":
...
async def cmd_set_plan(message: types.Message):
    ...
    language = get_language(target_user_id) or "ru"
    if language == "ru":
...
async def cancel_broadcast(callback: types.CallbackQuery):
    admin_lang = get_language(ADMIN_ID) or "ru"
...
async def handle_replenish_amount_input(message: types.Message):
    ...
    try:
        ...
        payment_url, transaction_id = await create_platega_payment(amount, user_id, "Balance replenishment")
        ...
        text = await get_replenish_payment_text(language, amount, transaction_id, paid=False)
        
        sent_message = await send_photo(
            message.chat.id,
            text,
            await get_payment_keyboard(language, transaction_id, paid=False, is_replenish=True),
            image_path
        )
...
async def handle_premium_plan(callback: types.CallbackQuery):
    ...
    payment_url, transaction_id = await create_platega_payment(PRICE, user_id, "Premium access")
    ...
    if language == "ru":
        ...
        text = (f"{MONEY_FLY} <b>Your payment invoice has been successfully created</b>\n\n"
    ...
    sent_message = await bot.send_message(
        ...
        text,
        reply_markup=await get_premium_payment_keyboard(language, transaction_id, paid=False),
        parse_mode="HTML"
    )
...
async def handle_activate_premium_internal(callback: types.CallbackQuery):
    ...
    if user_id not in payment_data or payment_data[user_id]["transaction_id"] != transaction_id:
        msg = "❌ Payment not found" if language == "en" else "❌ Платёж не найден"
        await callback.answer(msg, show_alert=True)
        return
    ...
    if not payment_data[user_id].get("paid", False):
        ...
        if result and result[0] == 'completed':
            ...
            msg = "❌ Please pay the invoice first" if language == "en" else "❌ Сначала оплатите счёт"
            await callback.answer(msg, show_alert=True)
            return
    ...
    msg = "🎉 Premium activated for 30 days!" if language == "en" else "🎉 Премиум активирован на 30 дней!"
    await callback.answer(msg, show_alert=True)
...
async def cancel_premium_payment(callback: types.CallbackQuery):
    ...
    msg = "❌ Payment cancelled" if language == "en" else "❌ Оплата отменена"
    await callback.answer(msg, show_alert=True)
...
async def handle_replenish_balance(callback: types.CallbackQuery):
    ...
    await edit_photo(callback.message, text, await get_replenish_amount_keyboard(language), image_path)
    await callback.answer()
...
async def handle_confirm_replenish(callback: types.CallbackQuery):
    ...
    if not replenish_data[user_id].get("paid", False):
        ...
        if result and result[0] == 'completed':
            ...
            msg = "❌ Please pay first" if language == "en" else "❌ Сначала оплатите"
            await callback.answer(msg, show_alert=True)
            return
    ...
    msg = f"✅ Balance topped up by {amount} rub!" if language == "en" else f"✅ Баланс пополнен на {amount} руб!"
    await callback.answer(msg, show_alert=True)
...
async def main():
    asyncio.create_task(run_web_server_async())
