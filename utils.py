...
def init_db():
    ...
    # Синхронизируем user_balance с существующими пользователями
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    cursor.execute('''
...
def add_user(user_id, username, first_name, referrer_id=None):
    ...
    if not exists:
        ...
        # Добавляем в таблицу user_balance
        cursor.execute(
            'INSERT OR IGNORE INTO user_balance (user_id, balance, trial_used) VALUES (?, 0, 0)',
            (user_id,)
        )
        ...
        # Обновляем referrer только если он не был установлен
        if referrer_id:
        ...
        # Проверяем и добавляем запись в user_balance если её нет
        cursor.execute('SELECT user_id FROM user_balance WHERE user_id = ?', (user_id,))
        ...
        if not balance_exists:
            ...
            conn.commit()
            print(f"[DEBUG] Added missing balance for existing user {user_id}")
